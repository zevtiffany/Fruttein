...
function submitPreorder(e) {
    ...
    db.runTransaction((transaction) => {
        ...
        // Poin dihitung berdasarkan TOTAL QTY item yang dipesan, bukan per PO
        const membersRef = db.collection('members');
        return membersRef.where('phone', '==', phoneClean).get().then(snapshot => {
            if (snapshot.empty) {
                // New member — poin = totalQty item
                const newMember = {
                    name: name.trim(),
                    phone: phoneClean,
                    points: totalQty,
                ...
                // Existing member — tambah poin & purchases sesuai qty item
                const memberDoc = snapshot.docs[0];
                const memberData = memberDoc.data();

                return membersRef.doc(memberDoc.id).update({
                    points: (memberData.points || 0) + totalQty,
                    purchases: (memberData.purchases || 0) + totalQty
                });
            }
        });
    }).then(() => {
...
async function processCancelPo(orderId, dateStr, qty, phone) {
    try {
        ...
        // 3. Reverse Member Points — dikurangi sesuai qty item yang dibatalkan
        if (phone) {
            const membersRef = db.collection('members');
            ...
            if (!memberSnapshot.empty) {
                ...
                const newPoints = Math.max(0, (memberData.points || 0) - qty);
                const newPurchases = Math.max(0, (memberData.purchases || 0) - qty);
...
function exportPoOrders(dateStr) {
    ...
    db.collection('po_orders')
        ...
        .then(snapshot => {
            ...
            let csvContent = "Waktu Order,Nama Pemesan,WhatsApp,Link Gmaps,Penanda Visual,Menu Dipesan,Total Item\n";
            ...
            docsData.forEach(data => {
                ...
                // Bangun daftar menu dari array items (cart)
                let menuList = '-';
                if (Array.isArray(data.items) && data.items.length > 0) {
                    menuList = data.items.map(item => `${item.name} (${item.qty}x)`).join('; ');
                } else if (data.productName) {
                    menuList = data.productName; // fallback data lama
                }
                const menu = `"${menuList.replace(/"/g, '""')}"`;

                csvContent += `${tsDate},${name},${phone},${gmaps},${landmark},${menu},${qty}\n`;
            });
